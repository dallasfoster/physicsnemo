...
- Adds `domain_parallel.shard_utils.halo_scatter`: a `torch.compile`-safe halo
  ...
  always-available fallback and correctness oracle) and a symmetric-memory
  (NVSHMEM/IPC) one-sided path that stages into a symmetric workspace and pulls each
  ...
  path is auto-selected when NVSHMEM is available or a one-time (cached, per-group)
...
- Adds a cross-node halo transport plus routing and dispatch support to
  `domain_parallel.shard_utils.halo_scatter`. `select_halo_backend` now routes by
  node locality (a symmetric-allocation-free hostname check): `funcol` for CPU or
  cross-node non-NVSHMEM groups, intra-node symmetric-memory, and a device-initiated
  NVSHMEM-Triton one-sided get/pull backend for cross-node NCCL groups. Adds
  `pack_halo_routing(cap=)` fixed-shape routing (for compiled `dynamic=False` runs)
  and an in-place `scatter_add_` / `index_add_` dispatch handler (correct value and
  gradient, eager and compiled). Two transport modes are opt-in and off by default
  (`PHYSICSNEMO_HALO_NVSHMEM_OVERLAP`, `PHYSICSNEMO_HALO_NVSHMEM_FLAGS`); the blocking
  path is the shipped default.
- Adds an opt-in `PHYSICSNEMO_TEST_FAST_EXIT` hook to the distributed test plugin that
  hard-exits after the coordinated cleanup and printed summary, sidestepping an at-exit
  NVSHMEM teardown fault; inert unless the env var is set.
